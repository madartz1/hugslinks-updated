...
if(!process.env.HUGS_VOTER_SESSION_SECRET||!process.env.RESEND_API_KEY||!process.env.HUGS_VOTER_FROM_EMAIL)return json({ok:false,error:"Member email sign-in is not configured yet."},503);
...
const res=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Authorization":"Bearer "+process.env.RESEND_API_KEY,"Content-Type":"application/json"},body:JSON.stringify({from:process.env.HUGS_VOTER_FROM_EMAIL,to:[email],subject:"Your HUGS Art Gallery sign-in code",text:"Your HUGS Art Gallery sign-in code is "+code+". It expires in 10 minutes. If you did not request it, ignore this message."})});
